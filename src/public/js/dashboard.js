...
async function loadStats() {
    try {
        ...
        document.getElementById("stat-services").textContent =
            services.length || 0;
        document.getElementById("stat-spareparts").textContent =
            spareParts.length || 0;
    ...
    try {
        const res = await fetch(endpoint, {
            ...
            headers: isFileUpload
                ? undefined
                : { "Content-Type": "application/json" },
            body: isFileUpload ? formData : JSON.stringify(data),
        });
...
function editRow(row) {
    ...
    if (delBtn && !delBtn.dataset.originalText)
        delBtn.dataset.originalText = delBtn.textContent;
